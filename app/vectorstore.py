...
class MemoryVectorStore:
    """
    Per-process FAISS / NumPy index mapping memory_id -> embedding, with a
    parallel id list for lookup. Thread-safe with a lock.
    """
    ...
    def __init__(self, dim: int = EMBEDDING_DIM):
        ...
        try:
            ...
            logger.info("FAISS native library unavailable (%s), using pure NumPy vector engine.", exc)
    ...
    def search(self, query: str, top_k: int = 5) -> list[tuple[str, float]]:
        ...
        with self._lock:
            if self._faiss_index is not None:
                scores, indices = self._faiss_index.search(vec, min(top_k, len(self._ids)))
                results = [
                ...
                top_indices = np.argsort(-scores)[:min(top_k, len(self._ids))]
