...
def _clean_llm_response(text: str) -> str:
    """Strips reasoning/thinking tags (e.g. <think>...</think>) from output."""
    import re
    if "<think>" in text:
...
def _call_groq(history: list[dict], model: str | None, system: str) -> str:
    import os
    from groq import Groq, GroqError, RateLimitError, NotFoundError, BadRequestError
    ...
    if not api_key:
        raise LLMError(
            ...
            "https://console.groq.com/keys and configure it in Render Environment Variables or Settings."
        )

    client = Groq(api_key=api_key)
    messages = [{"role": "system", "content": system}] + history

    # Candidate models to try in order on failure / rate limits / 404s
    primary = model or settings.groq_model or "groq/compound-mini"
    ...
    def _execute_candidates(models_to_try: list[str]) -> str | None:
        nonlocal last_error
        for cand_model in models_to_try:
            if cand_model in tried_models:
                continue
            tried_models.add(cand_model)
            try:
                ...
                logger.warning("Groq API error on model %s: %s. Trying next candidate model...", cand_model, exc)
                last_error = exc
                continue
            except GroqError as exc:
                err_str = str(exc).lower()
                if any(x in err_str for x in ["404", "not_found", "rate limit", "does not exist", "decommissioned", "400"]):
                    logger.warning("Groq API error on model %s (%s). Trying next candidate model...", cand_model, exc)
                    last_error = exc
                    continue
                ...
                logger.warning("Unexpected error on Groq model %s: %s", cand_model, exc)
                last_error = exc
                continue
        return None

    result = _execute_candidates(candidate_models)
    if result is not None:
        return result

    # Dynamic fallback: query available models from Groq API catalog
    try:
        remote_models = client.models.list()
        dynamic_candidates = []
        for m in remote_models.data:
            m_id = m.id
            if m_id not in tried_models and not any(skip in m_id.lower() for skip in ["whisper", "guard", "orpheus", "safeguard"]):
                dynamic_candidates.append(m_id)
        if dynamic_candidates:
            logger.info("Attempting dynamically discovered Groq models: %s", dynamic_candidates)
            dyn_result = _execute_candidates(dynamic_candidates)
            if dyn_result is not None:
                return dyn_result
    except Exception as fetch_exc:
        logger.warning("Failed to dynamically fetch Groq models list: %s", fetch_exc)

    if last_error:
        raise LLMError(f"All Groq models failed: {last_error}") from last_error
